...
| 2026-09-07 | CodeQL Default Setup & Actions Exclusion | Removed conflicting `.github/workflows/codeql.yml` to resolve GitHub Code Scanning upload rejection (`CodeQL analyses from advanced configurations cannot be processed when the default setup is enabled`). Updated Default Setup configuration to target `["javascript", "javascript-typescript", "typescript"]` and exclude `actions` (preventing exit code 32 on empty actions), ensuring all automated CodeQL checks pass cleanly. | **Accepted** |
